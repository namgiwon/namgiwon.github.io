...
* 옵션을 사용하지 않으면 EntityGraph를 써야 한다는데, 초반 하이버네이트에 대한 이해 부족으로 한 도메인에 OneToMany 자식 엔티티를 무지하게 설정해 놓은 관계로 도메인 관계 재설정을 해야만 한다. (EntityGraph는 OneToMany 하나 밖에 적용할 수 없다.)
